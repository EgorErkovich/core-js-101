...
/**
* Retcan be built with the specified sides a,b,c and false in any other ways.
*
* @param {number} a
* @param {number} b
* @param {number} c
...
function doRectanglesOverlap(rect1, rect2) {
  const tmp1 = rect1;
  const tmp2 = rect2;
  tmp1.rightBot = tmp1.top + tmp1.height;
  tmp2.rightBot = tmp2.top + tmp2.height;
  tmp1.len = tmp1.left + tmp1.width;
  tmp2.len = tmp2.left + tmp2.width;
  return (tmp1.left < tmp2.len && tmp1.len > tmp2.left
  && tmp1.top < tmp1.rightBot && tmp1.rightBot > tmp2.top);
}
...
function findFirstSingleChar(str) {
  ...
  while (str1.length) {
    ...
    if (str1.match(reg).length === 1) return char;
    str1 = str1.replace(new RegExp(char, 'g'), '');
  }
  return null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const begin = isStartIncluded ? `[${a1}, ${b1}` : `(${a1}, ${b1}`;
  return isEndIncluded ? `${begin}]` : `${begin})`;
}
